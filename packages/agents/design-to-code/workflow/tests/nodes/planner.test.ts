...
import { plannerAgent } from '../../src/nodes/planner.js';
...
describe('plannerAgent', () => {
  ...
  it('sets currentStep to PlannerAgent', async () => {
    setLLMClient(makeMockLLM(JSON.stringify(mockExecutionPlan)));
    const result = await plannerAgent(makeBaseState());

    expect(result.currentStep).toBe('PlannerAgent');
  });
  ...
  it('throws if the LLM returns invalid JSON', async () => {
    ...
    await expect(plannerAgent(makeBaseState())).rejects.toThrow();
  });
